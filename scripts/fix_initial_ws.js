...
const targetStr = \`export const createInitialWorldState = (randomize = true): WorldState => {
  const jitter = (base: number, range = 15): number =>
    randomize ? clamp(base + (Math.random() * range * 2 - range), 0, 100) : base;

  return {\`;

const replacementStr = \`export const createInitialWorldState = (randomize = true): WorldState => {
  const jitter = (base: number, range = 15): number =>
    randomize ? clamp(base + (Math.random() * range * 2 - range), 0, 100) : base;
...
// We have to replace from the start of the function up to the end of the return object
const startIdx = code.indexOf('export const createInitialWorldState');
const endIdx = code.indexOf('};', startIdx + 100);

if (startIdx !== -1 && endIdx !== -1) {
  const oldPart = code.substring(startIdx, endIdx + 2);
  const newPart = replacementStr + '\\n  };\\n};';
  code = code.replace(oldPart, newPart);
  fs.writeFileSync(file, code, 'utf8');
  console.log('Fixed createInitialWorldState');
} else {

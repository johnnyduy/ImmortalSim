...
### 1. Quy Tắc Giao Diện (UI Text Localization Rule)
**BẮT BUỘC:** Mọi đoạn text hiển thị cho người chơi (tiêu đề, nút bấm, thông báo, nhật ký sự kiện, v.v.) ĐỀU PHẢI có một ID duy nhất và được lưu trong các file ngôn ngữ `locales/[lang]/ui.json`. Code UI (các file `.tsx`) tuyệt đối không được chứa hardcoded string mà chỉ được gọi text thông qua object `uiText[language]?.['text_id']`.

### 2. Luồng dữ liệu và Tương tác giữa các Phân hệ
```mermaid
graph TD
    %% Subsystems

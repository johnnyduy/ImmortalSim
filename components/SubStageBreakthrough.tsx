...
export default function SubStageBreakthrough({
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black select-none overflow-hidden">
      ...
      {phase === 'success' && (
        <div className="relative max-w-md w-full mx-4 bg-[#14110f]/95 border-2 border-[#c5a059] rounded-lg p-6 md:p-8 flex flex-col items-center text-center shadow-[0_0_40px_rgba(197,160,89,0.35)] animate-scale-bounce overflow-hidden">
          {/* Ancient Frame Corners */}
          <div className="absolute top-2 left-2 w-4 h-4 border-t-2 border-l-2 border-[#c5a059]/60" />
          <div className="absolute top-2 right-2 w-4 h-4 border-t-2 border-r-2 border-[#c5a059]/60" />
          <div className="absolute bottom-2 left-2 w-4 h-4 border-b-2 border-l-2 border-[#c5a059]/60" />
          <div className="absolute bottom-2 right-2 w-4 h-4 border-b-2 border-r-2 border-[#c5a059]/60" />

          {/* Runic Rotating Background Ring */}
          <div className="absolute w-[260px] h-[260px] top-6 opacity-10 border-2 border-dashed border-[#c5a059] rounded-full animate-runic-spin pointer-events-none" />
          <div className="absolute w-[240px] h-[240px] top-8 opacity-5 border border-dashed border-[#c5a059] rounded-full animate-runic-spin-reverse pointer-events-none" />

          {/* Breakthrough Character Avatar (Centerpiece) */}
          <div className="relative w-44 h-64 md:w-48 md:h-72 rounded-sm overflow-hidden border border-[#c5a059]/40 bg-[#0f0c0a] shadow-lg shadow-black/80 animate-float-character">
            ...
            <div className="absolute inset-1.5 border border-[#c5a059]/20 pointer-events-none" />
          </div>

          {/* Success Header */}
          <div className="mt-5 space-y-1">
            <h2 className="font-serif text-2xl md:text-3xl font-extrabold tracking-widest text-[#e5c17b] drop-shadow-[0_0_12px_rgba(229,193,123,0.6)]">
              {isVi ? 'ĐỘT PHÁ THÀNH CÔNG!' : 'BREAKTHROUGH SUCCESS!'}
            </h2>
            <p className="text-[10px] md:text-xs uppercase tracking-widest text-cyan-400 font-bold">
              {isVi ? 'TIỂU CẢNH GIỚI TIẾN GIAI' : 'SUB-STAGE ADVANCEMENT'}
            </p>
          </div>

          {/* Transition Row (Old Stage -> New Stage) */}
          <div className="mt-4 flex items-center justify-center gap-4 bg-[#1e1915]/60 px-5 py-2.5 rounded-sm border border-[#3e3328]/50 min-w-[240px]">
          ...
          <button
            type="button"
            onClick={onClose}
            className="mt-6 w-full py-3 bg-[#c5a059] hover:bg-[#e5c17b] active:bg-[#a07f43] text-black font-bold font-serif text-sm tracking-widest rounded-sm shadow-md shadow-black/40 transition-all duration-300 relative group overflow-hidden cursor-pointer"
          >
